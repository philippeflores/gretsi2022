...
D'un point de vue analyse de données, un cytomètre produit un nuage de points dans un espace à $M$ dimensions. Le but est de séparer et identifier les différentes populations de cellules. L'analyse conventionnelle, basée sur une succession d'étapes en 2 dimensions, devient incomplètes, subjectives et coûteuse en temps plus le nombre de paramètres augmente. De ce fait, des méthodes automatiques sont apparues \cite{Qiu2011,Amir2013,VanGassen2015} mais sont coûteuses et peuvent difficilement être appliquées à de grands ensemble de données. De plus, ces méthodes mènent à des résulats insatisfaisants pour des populations de cellules rares et ont des visualisations limitées en grandes dimensions.

Dans ce papier, nous présentons une nouvelle méthode probabiliste appelée CTFlowHD. Pour faire face à la malédiction de la dimension, nous supposons un modèle bayesien naïf pour la densité conjointe multivariée. Ainsi, estimer l'histogramme en $M$ dimensions revient à estimer les facteurs d'un modèle tensoriel CP \cite{Harshman1970} dont la complexité demeure linéaire avec le nombre de dimensions. En suivant \cite{Kargas2018}, l'estimation du modèle CP d'ordre $M$ est formulé comme un problème d'approximation couplée des marginales 3D. Pour diminuer encore les coûts de calcul, nous proposons de considérer seulement une partie des marginales 3D lors du couplage. Enfin, pour améliorer l'interprétation de nos résultats, nous introduisons une étape de clustering supplémentaire qui classe directement les facteurs de rang 1.
...
Le Modèle Bayesien Naïf (MBN) introduit une variable latente discrète $L$, tel que les éléments de $\vect{x}$ sont conditionnellement indépendant par rapport à $L$ : \begin{equation}
    \label{eq:nbmcont}
